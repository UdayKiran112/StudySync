#!/usr/bin/env python3
"""
Load attendance (check-in/check-out per student/date) from the daily
...
-----------------------------
  ID NO             -> students.student_id (existing row; the FK, never
                        re-derived from Name)
  Date, IN, OUT     -> attendance (one row per student/date).
                        duration_minutes is always DERIVED from IN/OUT,
                        never read from the CSV's own DURATION column (it
                        disagreed with IN/OUT often enough, and can't
                        express the lunch rule below). Any overlap with
                        13:00-14:00 is excluded from duration_minutes as
                        an unattended lunch break, even for a session that
                        spans across it.
...
---------------------------------------------
  ...
  - Rows with no parseable Date, INCLUDING rows where the date parses
    structurally but is implausible for a historical attendance log:
    before 2005 (this library's earliest real join_date), or after today.
    The source CSV contains literal data-entry typos such as '14.07.2048'
    -- well-formed digits, nonsense year -- so this bound catches those.
  - Rows with no parseable check-in time (can't derive a session).
...
COL_IN = 5
COL_OUT = 6
# COL 7 = DURATION -- intentionally never read, see module docstring.
...
def main():
    ...
    with args.csv.open(encoding="utf-8-sig", newline="") as f:
        ...
        for line_no, row in enumerate(reader, start=1):
            ...
            duration, _lunch_overlap = compute_duration_minutes(check_in, check_out)
            ...
            try:
                ...
                skips.append(f"line {line_no}: attendance insert failed ({e}) -> SKIPPED")
