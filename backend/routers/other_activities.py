...
from models.other_activities import (
    ...
    OtherActivityAttendanceCreate,
    OtherActivityAttendanceResponse,
)
from security import require_api_key

logger = logging.getLogger("studysync.other_activities")

router = APIRouter(
    prefix="/api/other-activities",
...
def activity_row(db, id):
    row = db.execute(
        "SELECT * FROM other_activities WHERE activity_id=?", (id,)
    ).fetchone()
    if not row:
...
def attendance_row(db, id):
    row = db.execute(
        """SELECT a.*, COALESCE(s.name, p.name) participant_name, p.village, p.phone 
...
def add_activity(
    p: OtherActivityCreate, db: sqlite3.Connection = Depends(get_db_dependency)
):
    c = db.execute(
        "INSERT INTO other_activities(session_name, speaker_name, session_date, session_type, notes) VALUES(?,?,?,?,?)",
...
def get_activities(
    ...
    return [
        dict(x)
        for x in db.execute(
            q + " ORDER BY session_date DESC, activity_id DESC", args
        ).fetchall()
    ]
...
def update_activity(
    activity_id: int,
    p: OtherActivityUpdate,
    db: sqlite3.Connection = Depends(get_db_dependency),
):
    activity_row(db, activity_id)
    db.execute(
        "UPDATE other_activities SET session_name=?, speaker_name=?, session_date=?, session_type=?, notes=? WHERE activity_id=?",
        (
            p.session_name,
            p.speaker_name,
            p.session_date,
...
def delete_activity(
    activity_id: int, db: sqlite3.Connection = Depends(get_db_dependency)
):
    activity_row(db, activity_id)
...
@router.get(
    "/{activity_id}/attendance", response_model=List[OtherActivityAttendanceResponse]
)
def get_attendance(
    activity_id: int, db: sqlite3.Connection = Depends(get_db_dependency)
):
    ...
    LEFT JOIN students s ON s.student_id=a.student_id 
    LEFT JOIN external_participants p ON p.external_participant_id=a.external_participant_id 
    WHERE a.activity_id=? ORDER BY participant_name""",
            (activity_id,),
        ).fetchall()
    ]
...
@router.post(
    ...
    response_model=OtherActivityAttendanceResponse,
    status_code=201,
)
def add_attendance(
    activity_id: int,
    p: OtherActivityAttendanceCreate,
    db: sqlite3.Connection = Depends(get_db_dependency),
):
    activity_data = activity_row(db, activity_id)
    c = db.execute(
        "INSERT INTO other_activities_attendance(activity_id, participant_type, student_id, external_participant_id) VALUES(?,?,?,?)",
    ...
    if p.student_id:
        try:
            ...
            _cleanup_auto_filled_offline_if_needed(
                db, p.student_id, activity_data["session_date"]
            )
        except Exception:
            logger.exception(
                "Auto-fill cleanup failed for student %s on activity %s",
                p.student_id,
                activity_id,
            )
...
def delete_attendance(
    ...
    if row["activity_id"] != activity_id:
        raise HTTPException(404, "Attendance record not found")
    db.execute(
        "DELETE FROM other_activities_attendance WHERE attendance_id=?",
        (attendance_id,),
    )
