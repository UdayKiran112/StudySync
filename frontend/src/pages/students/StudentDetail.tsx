...
import { ArrowLeft, Mail, Phone, MapPin, Cake, CalendarDays } from "lucide-react";
...
export function StudentDetail() {
  ...
  if (isError || !student) return <ErrorBanner message={extractErrorMessage(error)} />;
    ...
    <div>
      ...
      <div className="mb-6 rounded-lg border border-border bg-card p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-2 text-xs text-slate">
                <CalendarDays size={13} /> Joined {formatDate(student.join_date)}
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 border-t border-border pt-4 text-sm sm:grid-cols-2 lg:grid-cols-4">
          ...
          <InfoItem icon={Cake} label="Date of birth" value={formatDate(student.date_of_birth)} />
        </div>
      </div>
        ...
        {TABS.map((t) => (
          <button
            ...
            className={clsx(
              ...
              tab === t.key ? "border-brass text-ink" : "border-transparent text-slate hover:text-ink"
            )}
          >
            {t.label}
          </button>
