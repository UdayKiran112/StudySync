...
import { isLibraryHoliday, toIsoDate, HOLIDAY_RULE_DESCRIPTION } from "../../../lib/holidays";
...
export function AttendanceCalendar({ history, weeks = 12 }: { history: Attendance[]; weeks?: number }) {
  const columns = useMemo(() => {
    ...
    while (cursor <= today) {
      ...
      const status: DayStatus = isLibraryHoliday(cursor) ? "holiday" : attendedDates.has(iso) ? "present" : "absent";
      days.push({ date: new Date(cursor), iso, status });
            ...
            <div key={wi} className="flex flex-col gap-1">
              ...
              {week.map((day) => (
                <div
                  ...
                  title={`${formatDate(day.iso)} — ${STATUS_LABEL[day.status]}`}
                />
              ))}
            </div>
          ))}
