...
export function SubscriptionSummaryDashboard({
  summary,
  loading,
}: {
    ...
    <section className="mb-6 space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="h-64 rounded-lg border border-border bg-card p-4">
          ...
          {summary?.usage_by_subscription.length ? (
            <ResponsiveContainer width="100%" height="88%">
              <BarChart
                data={summary.usage_by_subscription}
                layout="vertical"
                margin={{ left: 8 }}
              >
                <XAxis
                ...
                <Bar dataKey="count" fill="#a9782f" radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <p className="mt-8 text-sm text-slate">
              No subscription usage has been recorded yet.
            </p>
          )}
        </div>
        <div className="h-64 rounded-lg border border-border bg-card p-4">
          ...
          {summary?.type_distribution.length ? (
            <>
              <ResponsiveContainer width="100%" height="80%">
                <PieChart>
                  <Pie
                    ...
                    paddingAngle={2}
                  >
                    {summary.type_distribution.map((item, index) => (
                      <Cell
                        key={item.type}
            ...
            <p className="mt-8 text-sm text-slate">
              No type information has been added yet.
            </p>
          )}
        </div>
      </div>
