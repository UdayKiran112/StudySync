...
if (Test-Path $envFile) {
    ...
    # Fresh install: ship the operator's default ZKTeco device settings so a
    # new staff system starts with the same integration as the main venue
    # server. ZK_DEVICE_IP is deliberately left unset - it is venue-specific
    # and must be added per machine (device features stay disabled until then).
    $extraLines = @(
        "ZK_INTEGRATION=both"
        "ZK_DEVICE_PORT=4370"
        "ZK_COMM_KEY=0"
        "ZK_ATTENDANCE_MODE=poll"
        "ZK_POLL_INTERVAL=3"
        "ZK_ADMS_DELAY_SECONDS=10"
        "ZK_ADMS_ERROR_DELAY_SECONDS=30"
